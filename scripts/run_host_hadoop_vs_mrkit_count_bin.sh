...
REDUCERS="${REDUCERS:-4}"
ROUNDS="${ROUNDS:-3}"
MR_PORT_BASE="${MR_PORT_BASE:-19500}"

OUTROOT="${OUTROOT:-$ROOT/benchmark/hadoop-host}"
RUN_ID="${RUN_ID:-results_bin_$(date +%Y%m%d_%H%M%S)}"
...
mkdir -p "$OUTDIR" "$PLUGIN_DIR" "$BIN_DIR"
...
echo "building mrkit binary and plugin"
GOCACHE="$ROOT/.cache/go-build" \
GOMODCACHE="$ROOT/.cache/go-mod" \
GOTMPDIR="$ROOT/.cache/go-tmp" \
"$GO_BIN" build -o "$BIN_DIR/mrkit_legacy_main" "$ROOT/cmd/legacy/main/main.go"
...
for round in $(seq 1 "$ROUNDS"); do
  ...
  mr_log="$OUTDIR/mrkit_bin_round${round}.log"
  start_ms="$(ms_now)"
  if "$BIN_DIR/mrkit_legacy_main" \
      -i "$INPUT" \
    ...
    echo "$REDUCERS,$round,mrkit_bin,$ROWS,$wall_ms,$rps,$out_rows,PASS" >>"$CSV"
  else
    echo "$REDUCERS,$round,mrkit_bin,$ROWS,0,0,0,FAIL" >>"$CSV"
  fi
